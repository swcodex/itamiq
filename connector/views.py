...
def add_job(request):
    job_form = JobForm()
    script_formset = ScriptFormSet(initial=[{'order_exec': 1}])

    return render(request, 'pages/connector/add_job.html', {
        'job_form': job_form,
...
def add_job_api(request):
    if request.method == 'POST':
        ...
        if job_form.is_valid() and script_formset.is_valid():
            ...
            return HttpResponse('<div class="success">Job saved successfully</div>')
        else:
            # Collect form errors
            job_form_errors_html = '<div class="error">'
            for field, errors in job_form.errors.items():
                job_form_errors_html += f'<p>{field}: {errors.as_text()}</p>'
            job_form_errors_html += '</div>'

            script_formset_errors_html = '<div class="error">'
            for form in script_formset:
                for field, errors in form.errors.items():
                    script_formset_errors_html += f'<p>{field}: {errors.as_text()}</p>'
            script_formset_errors_html += '</div>'

            # Combine both form and formset errors into one HTML string
...
def edit_job(request, job_id):
    ...
    if request.method == 'POST':
        ...
        if job_form.is_valid() and script_formset.is_valid():
            ...
            for script in scripts:
                ...
                if script.id:
                    ...
                    if old_script.table_name != script.table_name:
                        logger.info(
                            f"Table name changed for script {script.id}. Old: {old_script.table_name}, New: {script.table_name}")
                        ...
                        if script.table_name:
                            logger.debug(
                                f"Creating new Table for script {script.id} with table_name: {script.table_name}")
            ...
            changes_made = Script.reorder_scripts(
                job.id)  # re-order the order_exec values in case they're incorrect or non-unique
...
def edit_table(request, table_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and column_formset.is_valid():
            ...
            return render(request, 'pages/connector/edit_table.html',
                          {'form': form, 'column_formset': column_formset, 'table': table, 'script': script})
